...
class Slide {
    ...
    async addImage(config) {
        ...
        try {
            ...
            this.updateRenderedShapesAfterAdd(image);
    ...
    addText(config) {
        try {
            ...
            this.updateRenderedShapesAfterAdd(textBox);
    ...
    addShape(config) {
        try {
            ...
            this.updateRenderedShapesAfterAdd(shape);
    ...
    async addChart(config) {
        try {
            ...
            this.updateRenderedShapesAfterAdd(chart);
    ...
    updateRenderedShapesAfterAdd(shape) {
        if (this.renderedShapes) {
            const rendered = this._createShape(shape.content, this.getNumElements() - 1);
            if (rendered) {
                this.renderedShapes.push(rendered);
            }
        }
    }
    ...
    getShapes() {
        if (!this.renderedShapes) {
            this.renderedShapes = [
                ...this._getChildShapes(this.content.get('p:cSld/p:spTree'))
            ];
        }
        return this.renderedShapes;
    }

    _getChildShapes(element) {
        const shapes = [];
        element.forEach((child, index) => {
            if (child.name() === 'p:grpSp') {
                shapes.push(...this._getChildShapes(child));
            } else {
                let shape = this._createShape(child, index);
                if (shape) {
                    shapes.push(shape);
                }
            }
    ...
    getShape(index) {
        if (this.renderedShapes) {
            const shapeIndex = index - (this.getNumElements() - this.renderedShapes.length);
            return this.renderedShapes[shapeIndex];
        } else {
            return this._createShape(this.content.get('p:cSld/p:spTree').at(index), index);
        }
    }

    insertElement(index, element) {
        const treeNode = this.content.get('p:cSld/p:spTree');
        const elementCount = treeNode.childCount();
        treeNode.insert(index, element);
        
        if (this.renderedShapes) {
            const shapeIndex = index - (elementCount - this.renderedShapes.length);
            const shape = this._createShape(treeNode.at(index), shapeIndex);
            if (shape) {
                this.renderedShapes.splice(shapeIndex, 0, shape);
            }
            for (let i = shapeIndex + (shape ? 1 : 0); i < this.renderedShapes.length; i++) {
                this.renderedShapes[i].index++;
            }
        }
    }

    removeShape(shape) {
        this.removeElementAt(shape.index);
        return shape;
    }
    ...
    removeElementAt(index) {
        const treeNode = this.content.get('p:cSld/p:spTree');
        const elementCount = treeNode.childCount();
        const removed = treeNode.removeAt(index);

        if (this.renderedShapes) {
            let shapeIndex = index - (elementCount - this.renderedShapes.length);
            this.renderedShapes.splice(shapeIndex, 1);

            for (let i = shapeIndex; i < this.renderedShapes.length; i++) {
                this.renderedShapes[i].index--;
            }
        }
    ...
    replaceShape(index, shape) {
        const treeNode = this.content.get('p:cSld/p:spTree');
        treeNode.replace(index, shape.content);
        
        if (this.renderedShapes) {
            let shapeIndex = index - (treeNode.childCount() - this.renderedShapes.length);
            this.renderedShapes.splice(shapeIndex, 1, shape);
        }
        shape.index = index;
    }

    replaceElement(index, element) {
        const treeNode = this.content.get('p:cSld/p:spTree');
        treeNode.replace(index, element);

        if (this.renderedShapes) {
            const shape = this._createShape(treeNode.at(index), index);
            if (shape) {
                this.renderedShapes.splice(index, 1, shape);
            }
        }
    }
