...
class SlideFactory {
    ...
    duplicateSlide(existingSlideName, newSlideName) {
        const existingRelsKey = `ppt/slides/_rels/${existingSlideName}.xml.rels`;
        const existingSlideKey = `ppt/slides/${existingSlideName}.xml`;
        const newRelsKey = `ppt/slides/_rels/${newSlideName}.xml.rels`;
        const newSlideKey = `ppt/slides/${newSlideName}.xml`;

        this.content[newRelsKey] = this.content[existingRelsKey].clone();
        this.content[newSlideKey] = this.content[existingSlideKey].clone();
        return this.content[newSlideKey];
    }
    ...
    addImage(slide, image, imageObjectName, rId) {
        ...
        let newImageBlock = Xml.create('p:pic', Xml.createTree({
            'p:nvPicPr': {
                'p:cNvPr': { [Xml.ATTR_KEY]: { id: picObjectId, name: `${imageObjectName} ${picObjectId}`, descr: imageObjectName } },
        ...
        this.content[slideKey].get('p:cSld/p:spTree').push(newImageBlock);
