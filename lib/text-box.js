...
class RenderedTextBox extends ElementProperties {
    ...
    textCount() {
        return this.content.get('p:txBody').filter((child) => child.name() === 'a:p')?.length;
    }

    text(index) {
        index = index || 0;
        return this.content.get('p:txBody').filter((child) => child.name() === 'a:p')?.[index]?.get('a:r/a:t')?.text();
    }

    setText(text, index) {
        index = index || 0;
        this.content.get('p:txBody').filter((child) => child.name() === 'a:p')?.[index]?.get('a:r/a:t')?.setText(text);
    }

    addHyperlink(rId, index) {
        index = index || 0;
        this.content.get('p:txBody').filter((child) => child.name() === 'a:p')?.[index]?.get('a:r/a:rPr')?.push(Xml.create('a:hlinkClick', null, { 'r:id': rId }));
    }
}
