...
class RenderedTextBox extends ElementProperties {
    constructor(content, index, namespace = "p") {
        super();
        this.content = content;
        this.index = index;
        this.namespace = namespace;
        this.properties = this.content.get(`${namespace}:spPr/a:xfrm`);
    }

    class() {
        return ShapeClass.Text;
    }

    textCount() {
        return this.content.get(`${this.namespace}:txBody`).filter((child) => child.name() === 'a:p')?.length;
    }

    text(index) {
        index = index || 0;
        return this.content.get(`${this.namespace}:txBody`).filter((child) => child.name() === 'a:p')?.[index]?.get('a:r/a:t')?.text();
    }

    setText(text, index) {
        index = index || 0;
        this.content.get(`${this.namespace}:txBody`).filter((child) => child.name() === 'a:p')?.[index]?.get('a:r/a:t')?.setText(text);
    }

    addHyperlink(rId, index) {
        index = index || 0;
        this.content.get(`${this.namespace}:txBody`).filter((child) => child.name() === 'a:p')?.[index]?.get('a:r/a:rPr')?.push(Xml.create('a:hlinkClick', null, { 'r:id': rId }));
    }
}

module.exports.TextBox = TextBox;
module.exports.RenderedTextBox = RenderedTextBox;
